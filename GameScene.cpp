						...
						if (!gameClear_)
						{
							...
							if (shot_->IsEnableCreate())
							{
								...
								if (InputManager::GetInstance().IsTrgDown(KEY_INPUT_Z) || input)
								{
									...
									// 打撃音の調整
									ChangeVolumeSoundMem(255 * 60 / 100, SEAttack_);
		...
		if (shot_->IsShotState())
		{
			...
			if (IsCollisionRectCenter(shotPos, shotSize, enemyPos, enemySize))
			{
				...
				if (player_->GetLevel() == 3)
				{
					...
					// 打撃音の調整
					ChangeVolumeSoundMem(255 * 60 / 100, SE3Hit_);
				...
				if (player_->GetLevel() == 4)
				{
					...
					// 打撃音の調整
					ChangeVolumeSoundMem(255 * 60 / 100, SE4Hit_);
				...
				if (player_->GetLevel() == 5)
				{
					...
					// 打撃音の調整
					ChangeVolumeSoundMem(255 * 60 / 100, SE5Hit_);
	...
	if (enemyBoss_->IsGetAlive())
	{
		...
		if (enemyShot_->IsShotState())
		{
			...
			if (IsCollisionRectCenter(eShotPos, eShotSize, playerPos, playerSize))
			{
				...
				// 打撃音の調整
				ChangeVolumeSoundMem(255 * 50 / 100, SEBossHit_);
...
void GameScene::Actor(void)
{
	...
	if (isActor1_ && InputManager::GetInstance().IsTrgDown(KEY_INPUT_C) && isKey1_ || input)
	{
		isActor1_ = false;
	...
	if (isActor2_ && InputManager::GetInstance().IsTrgDown(KEY_INPUT_C) && isKey2_ || input)
	{
		isActor2_ = false;
	...
	if (isActor3_ && InputManager::GetInstance().IsTrgDown(KEY_INPUT_C) && isKey3_ || input)
	{
		isActor3_ = false;
	...
	if (isActor4_ && InputManager::GetInstance().IsTrgDown(KEY_INPUT_C) && isKey4_ || input)
	{
		isActor4_ = false;
